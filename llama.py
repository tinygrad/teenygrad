...
from teenygrad.helpers import getenv, DEBUG, dtypes
...
class TransformerBlock:
  ...
  def __call__(self, x:Tensor, start_pos:int, freqs_cis:Tensor, mask:Optional[Tensor]):
    ...
    return self.norm_output(x, output)
...
class LLaMa:
  ...
  def build(model_path, tokenizer_path, model_gen=1, model_size="7B", quantize=False):
    ...
    from teenygrad.state import torch_load, load_state_dict
...
if __name__ == "__main__":
  ...
  parser = argparse.ArgumentParser(description='Run LLaMA in teenygrad', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
