...
import numpy as np
from teenygrad.helpers import GRAPH, DEBUG, prod, getenv, DType, dtypes, flatten, ImageDType
from teenygrad.ops import Device, UnaryOps, BinaryOps, TernaryOps, ReduceOps, MovementOps, LoadOps, OpType, LazyOp
from teenygrad.shape.shapetracker import ShapeTracker, View, get_contraction
from teenygrad.shape.symbolic import Node

from teenygrad.runtime.lib import RawConst, RawBuffer
from teenygrad.runtime.ops_cpu import RawNumpyBuffer
...
class LazyBuffer:
  ...
  def __init__(self, device:str, st:ShapeTracker, optype:OpType, op:LazyOp, dtype:DType, src:Optional[RawBuffer]=None):
    ...
    if GRAPH >= 3:
      from teenygrad.graph import log_op
